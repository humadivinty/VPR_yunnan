...
void CMiniDumper::setMiniDumpFileName( void )
{
    ...
    StringCbPrintf( m_szMiniDumpPath,sizeof(m_szMiniDumpPath),	L"%s%s.%ld.dmp", m_szAppPath, m_szAppBaseName, currentTime );
#else
...
LONG CMiniDumper::writeMiniDump( _EXCEPTION_POINTERS *pExceptionInfo )
{
	...
	if( GetModuleFileName( NULL, m_szAppPath, _MAX_PATH ) )
	{
		...
		if( pSlash )
		{
            ...
            StringCbCopyW(m_szAppBaseName, sizeof(m_szAppBaseName),  pSlash + 1 );
#else
...
//		wcscat( szDbgHelpPath, L"DBGHELP.DLL");
        StringCbCopyW( szDbgHelpPath, sizeof(szDbgHelpPath),  m_szAppPath );
        StringCbCatW( szDbgHelpPath,sizeof(szDbgHelpPath),  L"DBGHELP.DLL");
#else
...
//			                      m_szMiniDumpPath);
            StringCbPrintf( szScratch, sizeof(szScratch),
                                  L"There was an unexpected error:\n\nWould you "
                                  L"like to create a mini-dump file?\n\n%s ",
                                  m_szMiniDumpPath);
#else
