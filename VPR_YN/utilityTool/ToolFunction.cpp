...
bool DeleteDirectory(char* strDirName)
{
    ...
    sprintf(strTempFileFind, sizeof(strTempFileFind), "%s//*.*", strDirName);
    ...
    while (IsFinded)
    {
        ...
        if (!tempFind.IsDots())
        {
            ...
            if (tempFind.IsDirectory())
            {
                ...
                sprintf(strTempDir, sizeof(strTempDir), "%s//%s", strDirName, strFoundFileName);
            ...
            else
            {
                ...
                sprintf(strTempFileName, sizeof(strTempFileName), "%s//%s", strDirName, strFoundFileName);
...
int CirclelaryDelete(const char* folderPath, int iBackUpDays)
{
    ...
    sprintf(myPath,sizeof(myPath),  "%s\\*", folderPath);
    ...
    while (bFinded)
    {
        ...
        {
            sprintf(DirectoryName, sizeof(DirectoryName), "%s", myFileFind.GetFileName().GetBuffer());
            if (myFileFind.IsDirectory())
            {
                ...
                if (iYear < Last_Year)
                {
                    ...
                    sprintf(chLog,sizeof(chLog), "年份小于当前年份，删除文件夹%s", DirectoryName);
                    printf(chLog);
                }
                else if (iYear == Last_Year)
                {
                    if (iMonth < Last_Month)
                    {
                        ...
                        char chLog[MAX_PATH] = { 0 };
                        sprintf(chLog,sizeof(chLog), "月份小于上一月，删除文件夹%s", DirectoryName);
                        printf(chLog);
                    }
                    else if (iMonth == Last_Month)
                    {
                        if (iDay < Last_Day)
                        {
                            ...
                            char chLog[MAX_PATH] = { 0 };
                            sprintf(chLog, sizeof(chLog), "日号小于指定天数，删除文件夹%s", DirectoryName);
                            printf(chLog);
                        }
                    }
                }
